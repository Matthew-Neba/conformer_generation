...
class PuffeRL:
    ...
    def train(self):
        ...
        for mb in range(num_minibatches):
            ...
            # Stop policy updates early when KL drifts too far from the behavior policy.
            if target_kl > 0.0 and approx_kl.item() > target_kl:
                break

        prof.mark(1)
        prof.elapsed(P.TRAIN, 0, 1)

        denom = max(1, minibatches_done)
        losses = {k: v.item() / denom for k, v in losses.items()}
        y_pred = val.flatten()
